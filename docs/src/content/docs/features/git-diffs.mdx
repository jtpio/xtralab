...
Before saving, the diff checks for changes on disk, including edits made by
running agents. If the file has changed, autosave pauses and asks whether to
overwrite it or keep the version on disk. Discarding a change inside the editor
can be undone with the editor's undo shortcut.
